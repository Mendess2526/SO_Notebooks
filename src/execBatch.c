...
static inline void execCommand(Command c, size_t i, Pipes inPipes, Pipes outPipes);

static inline void writeToPipes(Command cur, Pipes inPipes, char* buf, size_t n);
...
void execBatch(Command c, int* pipfd){
    ...
    for(size_t i = 0; i < chainedNum && cur; i++, cur = command_pipe(cur)){
        ...
        while((n = read(pipes_index(outPipes, i)[0], buf, 1024)) > 0){ // Read cmd output
            write(pipfd[1], buf, (size_t) n); // Write to parent pipe for storage
            if(i < (chainedNum - 1)){
                writeToPipes(cur, inPipes, buf, (size_t) n);
            }
        }
...
void writeToPipes(Command cur, Pipes inPipes, char* buf, size_t n){
